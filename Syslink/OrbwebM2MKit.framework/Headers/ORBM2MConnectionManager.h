...
typedef NS_ENUM(NSUInteger, ORBM2MErrorCode) {
    ...
    ORBM2M_CONN_CLIENT_CONNECT_TAT_FAIL,
    // clinet register nic fail.
    ORBM2M_CONN_CLIENT_REG_NIC_FAIL_RSP,
    
    ORBM2M_CONN_CLIENT_DOMAIN_ERROR = 3100,
    
    ORBM2M_CONN_CLIENT_CLOSE_CONNECT = 3101,
...
typedef NS_OPTIONS (NSUInteger, ORBM2MType) {
    ...
    /// P2P option
    ORBM2MType_TCP_UDP = ORBM2MType_TCP | ORBM2MType_UDP,
...
@property NSInteger mNetworkType;
 ...
 */
- (void)asyncConnectWithRDZ:(NSString *)rdz_path
                        SID:(NSString *)SID
...
- (void)startRenewTaskWithQueue:(NSString *)rdz_path
                               :(NSString*) SID
                withRemotePorts:(NSArray*) remotrPorts
                               :(BOOL)auth
                               :(int)type;
...
/** @brief Get specified type connection if connection exists.
 @param SID M2M Connection SID.
 @param specifiedType specify M2M type for p2p or relay
 @return ORBM2MConnection
 */
- (nullable ORBM2MConnection*)getConnectionWithSID:(NSString *)SID specified:(ORBM2MType)specifiedType;

/** @brief Get authorized connection if connection exists.
 @param SID M2M Connection SID.
 @return ORBM2MConnection
 */
- (nullable ORBM2MConnection*)getAuthorizedConnectionWithSID: (NSString*)SID;
...
- (void)endConnectionWithConnectionKey2:(NSString *)key;
...
//private function
//- (void) CraeteTCPConnect:(ORBM2MConnection *)conn;
...
extern NSString * const ORBM2MConnectionDidRefreshNotification2;
/**
